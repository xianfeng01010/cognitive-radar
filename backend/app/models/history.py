...
class PushHistory(Base):
    ...
    buffer_item_id = Column(UUID(as_uuid=True), ForeignKey("buffer_items.id"), nullable=True)
    push_tier = Column(String(20))
    content_snapshot = Column(JSONB, default={})
...
class SearchHistory(Base):
    ...
    search_strength = Column(String(10), default="medium")
    created_at = Column(DateTime, default=datetime.utcnow)
...
class DeleteLog(Base):
    ...
    deleted_ids = Column(JSONB, default=[])
    created_at = Column(DateTime, default=datetime.utcnow)